...
class hdf5_packager(packager):
    ...
    def package_image(self, image, timestamp, img_idx, exp_start=None, exp_end=None):
        ...
        image_dset.attrs['exp_start'] = exp_start
        image_dset.attrs['exp_end'] = exp_end

    def package_sharp_image(self, image, timestamp, img_idx):
        image_dset = self.events_file.create_dataset("sharp_images/image{:09d}".format(img_idx),
                data=image, dtype=np.dtype(np.uint8))
        image_dset.attrs['size'] = image.shape
        image_dset.attrs['timestamp'] = timestamp
        image_dset.attrs['type'] = "greyscale" if image.shape[-1] == 1 or len(image.shape) == 2 else "color_bgr" 
    ...
    def set_data_available(self, num_images, num_flow):
        if num_images > 0:
            ...
            self.events_file.create_group("sharp_images")
        if num_flow > 0:
