...
from utils.event_utils import binary_search_h5_dset, events_to_voxel_torch
...
class Dataset(base.Dataset):
    ...
    def __init__(self,opt,split="train",subset=None):
        ...
        self.root = opt.data.root or "data/event_blender"
        ...
        if opt.event and split=='train':
            self.event_path = "{}/{}/events".format(self.root,opt.data.scene)


        assert(not self.augment, "not support data augmentation")
    ...
    def __getitem__(self,idx):
        ...
        # load event
        # if self.split == 'train':
        #     voxel = self.get_event(opt, idx)
        #     sample.update(voxel=voxel)
        return sample
    ...
    def get_event(self, opt, idx):
        voxel = torch.from_numpy(np.load(os.path.join(self.event_path, f'{idx:09d}.npy'))).unsqueeze(0)
        return voxel
